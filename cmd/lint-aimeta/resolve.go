...
func runResolve(paths []string, opts aimeta.Options, stdout, stderr io.Writer) int {
	...
	for _, p := range paths {
		...
		if err != nil {
			_, _ = fmt.Fprintf(stderr, "lint-aimeta --resolve: %v\n", err)
			return exitError
		}
	...
	if len(fixedAll) > 0 {
		_, _ = fmt.Fprintf(stdout, "lint-aimeta --resolve: applied %d fix(es)\n", len(fixedAll))
		printText(stdout, fixedAll)
	} else {
		_, _ = fmt.Fprintln(stdout, "lint-aimeta --resolve: no fixable violations found")
	}
	...
	for _, p := range paths {
		...
		if err != nil {
			_, _ = fmt.Fprintf(stderr, "lint-aimeta --resolve re-lint: %v\n", err)
			return exitError
		}
