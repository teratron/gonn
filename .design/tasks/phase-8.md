...
- [ ] **T-8A01** — Implement `pkg/optimizer/exponential_lr.go`: `ExponentialLR[T]` —
...
- [ ] **T-8B01** — Create `cmd/gonn/` directory structure and `cmd/gonn/main.go`:
...
- [ ] **T-8B02** — Implement `trainCmd()` in `cmd/gonn/train.go`:
...
- [ ] **T-8B03** — Implement `queryCmd()` in `cmd/gonn/query.go` and `verifyCmd()` in
  `cmd/gonn/verify.go`.
...
- [ ] **T-8B04** — Implement `versionCmd()` in `cmd/gonn/version.go`: prints
...
- [ ] **T-8B05** — Dataset streaming integration in `cmd/gonn/csv.go`:
...
- [ ] **T-8T01** — LR Scheduling validation (after Track A):
...
- [ ] **T-8T02** — CLI integration tests (after Track B):
