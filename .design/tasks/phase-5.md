...
- [x] **T-5B03** — `pkg/nn/nn_test.go`: multi-hidden Compile + Fit tests for two-hidden, three-hidden, and seven-hidden chains. Single-hidden XOR convergence test stays in place as the "no-regression" baseline. `PresetMNIST` and `PresetRegression` Compile() now succeeds (v0.1 returned ErrUserConfig); add positive-path test for both. `go test -race -cover` ≥ 80 % on `pkg/nn`, no existing test regresses. **Changes:** `multihidden_test.go` adds table-driven `TestCompileFitMultiHiddenChainDepths` for depths {2,3,7} and `TestCompileMultiHiddenTwoHiddenConverges` (Sigmoid Xavier, 20000 epochs, target ≤ 0.10). pkg/nn coverage 85.2 %, race-clean. **Phase 4 regression note:** Initial Track A propagation rewrite stored δ on miss (spec §5.6 pseudocode), which broke `examples/callbacks` `TestTrainConverges` (single-hidden XOR loss > 0.15 in 2000 epochs). Reverted to v0.1 raw-miss + derivative-in-update pattern, extended positionally to multi-hidden — bit-identical for len==1, extends naturally to chain. `examples/callbacks` and all other v0.1 examples regression-clean.
