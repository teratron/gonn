...
- **Status:** Todo
- **Assignment:** Agent
- **Verify:** `go test -run TestEncoderBlock_BackwardFD -count=1 ./pkg/layer/transformer/` PASS — finite-difference gradient check (max_abs_err < 1e-4, `T=float64`) at `Dmodel=8, NumHeads=2, Dff=16, SeqLen=4` covers every child parameter group (4 MHA projections + biases, 2 FFN matrices + biases, 2 LayerNorm γ/β) + the block input gradient
