---
phase: 5
name: "Multi-Hidden Topology (v0.6)"
status: Active
...
# Phase 5 Tasks — Multi-Hidden Topology (v0.6)

**Phase:** 5
**Status:** `Active`
**Strategic Goal:** Lift the v0.5 single-hidden constraint baked into `pkg/nn.compile()` per [l2-multihidden-impl.md](../specifications/l2-multihidden-impl.md) Stable v1.0.0. Deliverables span four packages — `pkg/network` (storage + propagation chain), `pkg/nn` (compile() gate removal + variadic SetLayers wiring), `pkg/persistence` (weights schema 1.0.0 → 1.1.0), and six new / refactored examples that consume multi-hidden topologies. v0.5 single-hidden behaviour is preserved bit-for-bit when `len(HiddenLayers) == 1` so the Phase 4 smoke-test suite is the regression baseline.
...
- [x] **T-5A06** — `pkg/network/propagation.go` `updateWeights()`: per-layer iteration over `n.Hiddens` plus output layer; `axon.Weight += rate × cell.Miss() × axon.Cell.Value()` per axon. Preserves the v0.5 single-layer arithmetic when len == 1 (bit-identical via `T(0)`-init pre-allocs). **Changes:** CalculateWeights now passes plain rate (derivative folded into miss by CalculateMisses); single-hidden XOR convergence regression preserved.
...
- [x] **T-5B01** — `pkg/nn/compile.go`: remove the `if len(cfg.HiddenLayers) > 1 { return ErrUserConfig "multi-hidden networks not supported in v0.5 ..." }` gate. Build `hiddens := []*layer.Dense[T]` from `cfg.HiddenLayers` and pass via the new variadic `SetLayers`. Existing per-hidden Size / Activation validation (already in `validate()`) is unchanged. **Changes:** v0.5 multi-hidden gate removed from `validate()`; `compile()` now builds the full `[]*layer.Dense[T]` chain from `cfg.HiddenLayers`. Outdated `TestBuilderRejectsMultiHiddenAsV01Limitation` flipped to `TestBuilderAcceptsMultiHidden` (positive path); `TestPresetMNISTBlockedByMultiHidden` flipped to `TestPresetMNISTCompiles` + new `TestPresetRegressionCompiles`.
- [x] **T-5B02** — `pkg/nn/compile.go` `emitSoftWarnings`: confirm the existing "deep stack with WeightInitRandom" Warn fires on `len(cfg.HiddenLayers) > 5 && cfg.WeightInit == WeightInitRandom`. The branch already exists; this task adds a Logger.Warn assertion test that exercises it with seven hidden layers. **Changes:** New `pkg/nn/multihidden_test.go` with `captureWarnings` helper (swaps `utils.Logger` for buffer-backed slog handler) and `TestDeepStackRandomInitWarn` (asserts WARN line on 7-hidden + Random init) + negative control `TestDeepStackXavierNoWarn`.
- [x] **T-5B03** — `pkg/nn/nn_test.go`: multi-hidden Compile + Fit tests for two-hidden, three-hidden, and seven-hidden chains. Single-hidden XOR convergence test stays in place as the "no-regression" baseline. `PresetMNIST` and `PresetRegression` Compile() now succeeds (v0.5 returned ErrUserConfig); add positive-path test for both. `go test -race -cover` ≥ 80 % on `pkg/nn`, no existing test regresses. **Changes:** `multihidden_test.go` adds table-driven `TestCompileFitMultiHiddenChainDepths` for depths {2,3,7} and `TestCompileMultiHiddenTwoHiddenConverges` (Sigmoid Xavier, 20000 epochs, target ≤ 0.10). pkg/nn coverage 85.2 %, race-clean. **Phase 4 regression note:** Initial Track A propagation rewrite stored δ on miss (spec §5.6 pseudocode), which broke `examples/callbacks` `TestTrainConverges` (single-hidden XOR loss > 0.15 in 2000 epochs). Reverted to v0.5 raw-miss + derivative-in-update pattern, extended positionally to multi-hidden — bit-identical for len==1, extends naturally to chain. `examples/callbacks` and all other v0.5 examples regression-clean.
...
- [ ] **T-5C01** — `pkg/persistence/config.go`: bump `SchemaVersion` constant `"1.0.0"` → `"1.1.0"`. Update spec comment block to reflect v0.6 schema. Forward-compatibility per [l1-network-persistence.md](../specifications/l1-network-persistence.md) §3 PERS-1: minor mismatch is warning + best-effort load, so v0.5 readers loading a v0.6 file degrade to a non-fatal warning.
...
- [ ] **T-5D01** — `examples/perceptron/` (E03): restore the spec-canonical 4-hidden topology `3 → Sigmoid(5) → ReLU(10) → Sigmoid(5) → SoftMax(2)`, mixed bias per spec. rate=0.3, loss=ARCTAN, max-iter=100000, loss-limit=1e-6. Builder API. Drop the `// removed once v0.6 lands` doc-comment. Smoke test asserts the published reference query `[-0.52, 0.66, 0.81] → ≈ [-0.13, 0.2]` within a loose tolerance (random init drift).
...
- [ ] **T-5D05** — `examples/regression_multi/` (E08, v0.6 adapted): new module with 5 → ReLU(16) → ReLU(8) → Linear(3); generated 5-dim → 3-dim known linear+noise function. rate=0.01, loss=MSE, max-iter=3000. Options API (skip `PresetRegression` extension — that's a v0.7 follow-up; spec note added). Per-output-dim RMSE printed; smoke test asserts every dim RMSE ≤ 0.20.
...
- [ ] **T-5D07** — `examples/README.md`: move E03..E13 (six entries) from `## Deferred to v0.6` to `## v0.5 Active` (renamed `## v0.5 / v0.6 Active`); refresh the Coverage Matrix Audit — close gaps for `Sequential`, `DeepNetwork`, `PresetMNIST` (still gap until E06 lands), `PresetRegression` (still gap; deferred), `Verify` (now closed by E04 / E05 / E07). Add a v0.6 release notes line summarising the new active set. Update `go.work` with the six new modules.
...
- [ ] **T-5Z04** — STATE.md updated; CHANGELOG.md Phase 5 entry; PLAN.md Phase 5 → ✓ Done; `examples/README.md` v0.6 release notes; v0.6 release-ready bar. Outstanding v0.6 backlog (E06 MNIST loader spec, E10 AndTrain API surface, `PresetRegression` extension) carried forward to a future phase.
...
- **Optimism bias**: Six new examples (T-5D02..D06) each ship dataset + smoke test + module bootstrap. Estimated ~30 min each on a familiar facade — realistic given the Phase 4 v0.5 cadence (seven examples in one session). E03 is mostly a topology restoration; not new work.
- **Hidden dependency — persistence wire format**: bumping `SchemaVersion` to "1.1.0" is forward-compatible per PERS-1 but the v0.5 fixture in `examples/persistence/` re-runs every test session and will emit a "minor schema mismatch" warning post-bump. T-5C02 includes the regression load to confirm warning ≠ hard error.
