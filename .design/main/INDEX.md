...
| [l1-embedding-layers.md](specifications/l1-embedding-layers.md) | Embedding layers contract — TokenEmbedding (vocabulary lookup), PositionalEncoding (sinusoidal / learnable), EmbeddingStack additive composition; 10 invariants (EMB-1..10) including sparse-gradient update | Stable | L1 | 0.1.0 |
| [l2-embedding-impl.md](specifications/l2-embedding-impl.md) | Go realization of embedding layers — pkg/layer/embedding/ (TokenEmbedding[T] + single PositionalEncoding[T] with Mode enum + EmbeddingStack[T]); IDLayer[T] sub-interface + SparseParamAccessor[T] optimizer extension | Stable | L2 | 0.1.0 |
| [l1-transformer-block.md](specifications/l1-transformer-block.md) | Transformer block contract — EncoderBlock (bidirectional) + DecoderBlock (causal) + Stack(N); pre-norm vs post-norm construction switch; 10 invariants (TRANS-1..10) including residual identity and child-delegated persistence | Stable | L1 | 0.1.0 |
| [l2-transformer-impl.md](specifications/l2-transformer-impl.md) | Go realization of transformer blocks — pkg/layer/transformer/ (EncoderBlock[T] + DecoderBlock[T] + Stack[T]); typed child ownership (Attn / Norm1 / Norm2 / FFN1 / FFN2 / Drop1 / Drop2); composes MultiHeadAttention + LayerNorm + Dense + Dropout | Stable | L2 | 0.1.0 |
| [l1-quantization.md](specifications/l1-quantization.md) | Post-training quantization contract — affine int8 (symmetric / asymmetric) with per-tensor / per-channel granularity, calibration protocol (min-max / 99.9-percentile / entropy), .qnn.json sibling artifact, side-by-side evaluator; 10 invariants (QUANT-1..10); L2 deferred to Phase 19+ | Stable | L1 | 0.1.0 |
| [l2-quantization-impl.md](specifications/l2-quantization-impl.md) | Go realization of post-training int8 quantization — pkg/quantization/ (Quantize[T] + QuantizedNetwork[T] + QuantizedDense/Conv/AttnProjections + CalibrationRunner + SideBySideEvaluator + .qnn.json persistence); three-phase delivery α weight-only → β full-int8 → γ attention projections | Stable | L2 | 0.1.0 |

## Meta Information

- **Maintainer**: Core Team
- **Last Updated**: 2026-05-20 (magic-spec Spark 1: l2-quantization-impl v0.1.0 authored Stable — Go realization of l1-quantization.md; pkg/quantization/ package; 10 QUANT invariants mapped; three-phase plan α/β/γ; enables Phase 19 planning. INDEX v2.17.0 → v2.18.0.)
- **Previous Update**: 2026-05-19 (magic-spec Blank Trigger Creative Sparks 1+2+3: five new specs authored Stable via Trust Mode in a single session. **Spark 2 (Embedding)**: l1-embedding-layers v0.1.0 + l2-embedding-impl v0.1.0 — TokenEmbedding lookup with sparse-gradient EMB-9, sinusoidal + learnable PositionalEncoding (single struct with Mode enum), EmbeddingStack additive composition; new IDLayer[T] sub-interface for integer-ID input; new SparseParamAccessor[T] optimizer extension (SGD baseline); ErrVocabOutOfRange sentinel. **Spark 1 (Transformer)**: l1-transformer-block v0.1.0 + l2-transformer-impl v0.1.0 — EncoderBlock (bidirectional) + DecoderBlock (causal) + Stack(N) composing existing MultiHeadAttention + LayerNorm + Dense + Dropout; pre-norm vs post-norm TRANS-C6 construction switch; typed child ownership; persistence delegates to children per TRANS-9. **Spark 3 (Quantization)**: l1-quantization v0.1.0 (L1-only; L2 deferred to Phase 19+) — affine int8 with symmetric / asymmetric range, per-channel-for-weights granularity, three calibration strategies, .qnn.json sibling artifact, side-by-side evaluator, projection-only attention coverage in v0.1. All five specs satisfy MVC, no RULES.md conflicts, no circular dependencies, layer constraints satisfied (L2 parents Stable). Together they close three architecture gaps: NLP input lift (Embedding), modern sequence body (Transformer), deployment compression (Quantization). INDEX v2.16.0 → v2.17.0.)
