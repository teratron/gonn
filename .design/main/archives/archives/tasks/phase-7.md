...
provides:
  - LR scheduler interface (Scheduler[T], BindScheduler, Granularity enum)
  - StepLR, WarmUpLR, CosineAnnealingLR, ChainScheduler implementations
  - Bulk topology constructors (Repeat, Pattern, HiddenLayers) on Builder and Options APIs
  - GoNN developer skills (SKILL.md, examples, resources)
key_files:
  - pkg/optimizer/scheduler.go
  - pkg/optimizer/step_lr.go
  - pkg/optimizer/warmup_lr.go
  - pkg/optimizer/cosine_lr.go
  - pkg/optimizer/chain_scheduler.go
  - pkg/optimizer/scheduler_test.go
  - pkg/nn/builder.go
  - pkg/nn/options.go
  - pkg/nn/train.go
  - pkg/nn/config.go
  - pkg/nn/phase7_test.go
  - skills/gonn/SKILL.md
  - skills/gonn/examples/builder-xor.md
  - skills/gonn/examples/options-mnist.md
  - skills/gonn/examples/deep-network.md
  - skills/gonn/resources/api-reference.md
  - skills/gonn/resources/conventions.md
patterns_established:
  - Optional interface extension (LearningRateSetter[T]) for scheduler-to-optimizer bridging
  - BindScheduler wrapper pattern (wraps if LearningRateSetter implemented, no-op otherwise)
  - Granularity enum for per-epoch vs per-step scheduler dispatch in training loop
  - Bulk constructor append semantics (Repeat/Pattern) vs replace semantics (HiddenLayers)
  - Developer Skills directory structure (SKILL.md + examples/ + resources/)
...
- [x] **T-7A01** — Define `pkg/optimizer/scheduler.go`: `Scheduler[T Float]` interface with
...
- [x] **T-7A02** — Implement `pkg/optimizer/step_lr.go`: `StepLR[T]` — rate decays by `gamma`
...
- [x] **T-7A06** — Wire scheduler into `pkg/nn/train.go`: after each epoch (or step, based on
...
- [x] **T-7B01** — Add `Repeat(count, size uint, act activation.Type, bias bool) *NN[T]`
  builder method to `pkg/nn/builder.go`. Appends `count` identical `HiddenLayerSpec[T]`
  entries to `Config.HiddenLayers`.

- [x] **T-7B02** — Add `Pattern(block []HiddenLayerSpec[T], repeats uint) *NN[T]` builder
  method. Appends `block × repeats` entries. Validation: `repeats == 0` or `len(block) == 0`
  → error at Compile().

- [x] **T-7B03** — Add `HiddenLayers(layers []HiddenLayerSpec[T]) *NN[T]` builder method.
  **Replaces** any previously added hidden layers (setter, not appender).

- [x] **T-7B04** — Add Options-style constructors: `Repeat[T](count, size, act)`,
...
- [x] **T-7C01** — Create `skills/gonn/SKILL.md` with YAML frontmatter (name: gonn) and
...
- [x] **T-7C02** — Create `skills/gonn/examples/`: `builder-xor.md` (Style A XOR),
  `options-mnist.md` (Style B placeholder), `deep-network.md` (100-layer via Repeat/Pattern).

- [x] **T-7C03** — Create `skills/gonn/resources/`: `api-reference.md` (condensed public API),
  `conventions.md` (C25–C33 translated for AI agents without referencing RULES.md).

## Validation Tasks

- [x] **T-7T01** — Scheduler + Deep Builder validation (after Track A + B):
...
- [x] **T-7T02** — Skills validation (after Track C):
