...
**Version:** 2.21.0
**Project Version:** 0.11.0 released; 0.12.0 RC ready (Phase 14 Done — pending `git tag -a v0.12.0`); 0.13.0 RC ready (Phase 15 Done — pending `git tag -a v0.13.0`); 0.14.0 RC ready (Phase 16 Done — pending `git tag -a v0.14.0`); 0.15.0 RC ready (Phase 17 Done — pending `git tag -a v0.15.0`); 0.16.0 RC ready (Phase 18 Done — pending `git tag -a v0.16.0`); 0.17.0 target (Phase 19 active)
**Generated:** 2026-04-29
**Last Updated:** 2026-05-20 (v2.21.0 — Phase 17/18 mechanical drift fix: checkboxes + outcomes + archive link; Project Version updated to RC-ready state; Phase 19 activated)
**Based on:** .design/main/INDEX.md v2.18.0
...
### Phase 19 — Quantization Implementation (promoted to active Phase 19)

- [l1-quantization.md](specifications/l1-quantization.md) — Stable v0.1.0 — **promoted to active Phase 19** after Phase 18 T-18A01..A09 Done + l2-quantization-impl.md Stable v0.1.0 authored 2026-05-20
- [l2-quantization-impl.md](specifications/l2-quantization-impl.md) — Stable v0.1.0 — **promoted to active Phase 19**; decomposed into 14 atomic tasks (T-19A01..A11 + T01..T02 + Z01); scoped 2026-05-20 via `/magic-task`
...
**Tasks file:** [archives/tasks/phase-18.md](archives/tasks/phase-18.md)
...
- [x] **[A] Transformer Block Implementation** ([l1-transformer-block.md](specifications/l1-transformer-block.md) + [l2-transformer-impl.md](specifications/l2-transformer-impl.md)) [L1+L2, Stable v0.1.0 each] — Done 2026-05-20. **Phase α:** `pkg/layer/norm/layernorm.go` (`LayerNorm[T].Backward` + `ApplyGradSGD`, T-18A01); `pkg/layer/transformer/config.go` + `doc.go` (`TransformerConfig[T]` + `Mode` enum + `applyActivationInPlace` helper, T-18A02); `block.go` (`Block[T]` interface + `addInPlace` + internal FFN primitive, T-18A03); `encoder.go` (`EncoderBlock[T]` struct + post-norm `Forward` + `Init`, T-18A04; post-norm `Backward` + `GradSlots` + `ApplyGradSGD`, T-18A05; JSON + `SetPaddingMask` + interface assertions, T-18A06). **Phase β:** `encoder.go` (`PreNorm` branch per TRANS-3, T-18A07; three Dropout positions per TRANS-C7, T-18A08). **Phase γ:** `decoder.go` (`DecoderBlock[T]` causal, T-18A09); `stack.go` (`Stack[T]` + `NewStack` + sequential Forward/Backward, T-18A10); `pkg/nn/options.go` + `train.go` (four `With*` options → `ConvPrefix` + `applyConvBackward` switch cases, T-18A11).

**Phase 18 Outcome:** All 14 tasks Done; v0.16.0 RC ready (pending `git tag -a v0.16.0`). Single sequential track A: LayerNorm[T].Backward + ApplyGradSGD; TransformerConfig[T] + QuantMode + applyActivationInPlace; Block[T] interface + addInPlace + internal FFN primitive; EncoderBlock[T] post-norm Forward/Backward/Init/JSON + SetPaddingMask; pre-norm branch (PreNorm flag, TRANS-3); Dropout wiring (TRANS-C7); DecoderBlock[T] causal masking; Stack[T] N-block sequential chain + NewStack + JSON envelope; four pkg/nn options (WithEncoderBlock/WithDecoderBlock/WithEncoderStack/WithDecoderStack → ConvPrefix) + applyConvBackward switch cases. TRANS-6 residual identity + TRANS-3 pre-norm gradient-flow depth-12 tests green. Archived to archives/tasks/phase-18.md.

## Phase 19 — Quantization L2 Implementation (v0.17.0)

*Peer transformation package that consumes a trained `*nn.Network[T]` and produces a deployable
`QuantizedNetwork[T]` int8 artifact persisted as `.qnn.json`. Realizes `l2-quantization-impl.md §5
α-β-γ` as the new `pkg/quantization/` package — three exported top-level operations: `Quantize[T]`
(transforms a float network into a quantized one), `Load[T]` (loads a `.qnn.json` artifact), and
`Evaluate[T]` (side-by-side float vs quant comparison per QUANT-8). No changes to `pkg/nn/`,
`pkg/layer/`, or `pkg/compute/` in v0.1 — quantization is a peer transformation layer, not an
extension of the training-time graph (GC-3: read-only network input; GC-5: quantized types do NOT
implement `layer.Layer[T]`). Three delivery phases: α (weight-only Dense/Conv1D/Conv2D without
activation calibration — minimal deployable subset); β (CalibrationRunner[T] + full-int8 GEMM path
with int32 accumulator + SideBySideEvaluator); γ (attention projection matrices Wq/Wk/Wv/Wo —
gated on Phase 18 closeout T-18Z01; scoring + softmax remain float per QUANT-10). Coverage target
≥ 80% per C30.*

**Subsystem:** `pkg/quantization/` (new — `config.go`, `params.go`, `quantizer.go`, `dense.go`,
`conv.go`, `attn.go`, `calibration.go`, `persistence.go`, `evaluator.go`, `doc.go` + `*_test.go`);
`pkg/utils/errors.go` (additive: `ErrCalibTooFewSamples`, `ErrDegenerateRange` sentinels)
**Requires:** Phase 18 ✓ (v0.16.0 RC — `EncoderBlock[T]` + `DecoderBlock[T]` + `Stack[T]` landed; gate T-18Z01 closed); l1-quantization.md Stable v0.1.0 ✓; l2-quantization-impl.md Stable v0.1.0 ✓ (authored 2026-05-20); l2-layer-types.md Stable v0.1.0 ✓; l2-attention-impl.md Stable v0.1.0 ✓; l2-persistence-impl.md Stable v1.0.0 ✓
**Tasks file:** [tasks/phase-19.md](tasks/phase-19.md)
**Track order:** Single sequential track A following the L2 §5 three-phase plan — Phase α: A01 → A02 → A03 → (A04 ∥ A05) → A06; Phase β: A07 → A08 → A09; Phase γ: A10 → A11. T-19T01 after Phase α (A06); T-19T02 after Phase β+γ (A09 + A11); Gate T-19Z01 (v0.17.0 RC).
**@role:planner audit (2026-05-20):** **Optimism Bias** — 14 atomic tasks (11A + 2T + 1Z), consistent with the Phase 15/16/17/18 baseline of 14-15. Phase α (A01-A06) is the deployable gate — users can quantize Dense+Conv1D/2D weight-only after Phase α. β and γ are additive with no API breakage. **Hidden Dependencies** — (1) `Quantize[T]` accesses layer weights via type-switch on `net.Config().ConvPrefix` — direct struct field access per layer type (GC-3 same-module peer, `[NN-TRAIN]` canonical reference); (2) `roundHalfEven` is NOT in stdlib — T-19A02 implements it; L1 §4.2 golden-value test is the normative anchor; (3) `CalibrationRunner[T]` drives source network Forward to collect stats — `nil` calibSamples must be guarded in WeightOnly mode; (4) full-int8 GEMM (T-19A08) uses plain `int32` accumulation — no SIMD in v0.1 (GC-1, QUANT-9); (5) Phase γ attention quantization requires Phase 18 `MultiHeadAttention[T]` public fields (`Wq/Wk/Wv/Wo`). **Cascade Risk** — T-19A02 (`roundHalfEven` + affine math) is the most critical primitive: L1 §4.2 `quantize(1.0, p, 0)=42` is the single normative anchor; wrong implementation makes every quantized weight incorrect. Persistence round-trip (T-19A06 + T-19T01) is the second highest-leverage gate — int8 bytes must survive base64 encode/decode bit-exact.

- [ ] **[A] Quantization L2 Implementation** ([l1-quantization.md](specifications/l1-quantization.md) + [l2-quantization-impl.md](specifications/l2-quantization-impl.md)) [L1+L2, Stable v0.1.0 each] — **Phase α:** `pkg/quantization/config.go` + `doc.go` (`QuantMode`/`QuantizationConfig[T]`/`CalibrationProvenance`, T-19A01); `params.go` (`QuantizationParams` + `dequantize`/`quantize`/`roundHalfEven`/`computeSymmetric`/`computeAsymmetric`, T-19A02); `quantizer.go` (`Quantize[T]`/`QuantizedNetwork[T]`/type-switch/`Load[T]`/SHA-256 hash, T-19A03); `dense.go` (`QuantizedDense[T]` weight-only Forward, T-19A04); `conv.go` (`QuantizedConv1D[T]`/`QuantizedConv2D[T]` weight-only, T-19A05); `persistence.go` (`MarshalQNN`/`UnmarshalQNN`/`.qnn.json` base64 schema, T-19A06). **Phase β:** `calibration.go` (`CalibrationRunner[T]` + MinMax/Percentile99p9, T-19A07); `dense.go` amend (int32 accumulator GEMM, T-19A08); `evaluator.go` (`Evaluate[T]`/`EvaluationResult`, T-19A09). **Phase γ:** `attn.go` (`QuantizedAttentionProjections[T]`, T-19A10); `quantizer.go` amend (MHA type-switch case, T-19A11).
...
graph LR
  ...
  R1 --> S1[Phase 19 — Quantization L2 Implementation]
```
...
| 2.20.0 | 2026-05-20 | Phase 19 scoped: Quantization L2 Implementation (v0.17.0 target). Single-track A realizing `l2-quantization-impl.md §5 α-β-γ` as new `pkg/quantization/` peer package — `Quantize[T]`/`Load[T]`/`Evaluate[T]` top-level API; `QuantizedNetwork[T]` inference-only chain; `QuantizedDense[T]`/`Conv1D[T]`/`Conv2D[T]` weight-only (Phase α); `CalibrationRunner[T]` + full-int8 int32-accumulator GEMM (Phase β); `QuantizedAttentionProjections[T]` for Wq/Wk/Wv/Wo (Phase γ). 14 atomic tasks (T-19A01..A11 + T01..T02 + Z01). @role:planner audit: Optimism Bias — 14 tasks consistent with baseline; Phase α (A01-A06) is deployable gate. Hidden Dependencies — (1) layer weight access via type-switch on ConvPrefix using `[NN-TRAIN]` pattern; (2) `roundHalfEven` not in stdlib — L1 §4.2 golden value `q(1.0)=42` is normative anchor; (3) nil calibSamples guard for WeightOnly mode; (4) pure Go int32 GEMM (no SIMD per GC-1); (5) Phase γ gated on Phase 18 T-18Z01. Cascade Risk — T-19A02 (affine math) is most critical; persistence round-trip (T-19T01) is highest-leverage gate. Backlog: "Phase 19+ Quantization" promoted to active Phase 19 (both conditions met: T-18A01..A09 Done + l2-quantization-impl.md Stable v0.1.0). Build Order edge R1→S1 promoted from dashed to solid. Phase 18 row updated In Progress (9/14). Pre-Planning Stabilization: all 69 specs already Stable (zero Draft promotions). Based on INDEX.md v2.18.0. |
