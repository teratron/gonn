...
**Version:** 2.18.0
**Project Version:** 0.11.0 released; 0.12.0 RC ready (Phase 14 Done — pending `git tag -a v0.12.0`); 0.13.0 RC ready (Phase 15 Done — pending `git tag -a v0.13.0`); 0.14.0 RC ready (Phase 16 Done — pending `git tag -a v0.14.0`); 0.15.0 target (Phase 17 active)
...
### Phase 18 — Transformer Block Implementation (gated on Phase 17 T-17Z01)

- [l1-transformer-block.md](specifications/l1-transformer-block.md) — Stable v0.1.0 (composite block contract — EncoderBlock + DecoderBlock + Stack(N); pre-norm vs post-norm; 10 invariants TRANS-1..10)
- [l2-transformer-impl.md](specifications/l2-transformer-impl.md) — Stable v0.1.0 (`pkg/layer/transformer/` — typed child ownership; composes MultiHeadAttention + LayerNorm + Dense + Dropout; 3-phase plan α-β-γ in §6)

> **Phase 18 scoping conditions**: Phase 17 closeout gate T-17Z01 must close first. Transformer Block reuses `MultiHeadAttention[T]` from Phase 17 Track A as its inner primitive and consumes `EmbeddingStack[T]` output from Phase 17 Track B for end-to-end NLP pipelines. The L2 §6 3-phase plan (α encoder post-norm baseline → β pre-norm + Dropout → γ decoder + Stack) is ready to scope into Phase 18 as soon as Phase 17 lands the attention + embedding primitives. Activate with `/magic-task` after `/magic-run` reports Phase 17 Done.
...
graph LR
  ...
  Q1 -.-> R1[Phase 18 — Transformer Block Implementation, unlocked after Phase 17 gate]
