...
| [Phase 18](archives/tasks/phase-18.md) | Transformer Block Implementation — single sequential track A (`pkg/layer/transformer/` EncoderBlock[T] + DecoderBlock[T] + Stack[T] + internal FFN primitive + Block[T] interface + TransformerConfig[T]; composes MultiHeadAttention + LayerNorm + Dropout + activation; `pkg/layer/norm/` LayerNorm[T].Backward; four pkg/nn options → ConvPrefix), v0.16.0 RC | `Done (Archived)` (14/14 — all tasks Done; gate T-18Z01 closed 2026-05-20) |
| [Phase 19](archives/tasks/phase-19.md) | Quantization L2 Implementation — `pkg/quantization/` PTQ int8 (weight-only Dense/Conv1D/Conv2D Phase α; CalibrationRunner + full-int8 GEMM Phase β; attention projection matrices Phase γ; `Quantize[T]`/`Load[T]`/`Evaluate[T]` top-level API; `.qnn.json` persistence), v0.17.0 RC | `Done (Archived)` (0/14 — scoped 2026-05-20) |
...
- **Last Updated**: 2026-05-20 (Phase 19 scoped via /magic-task: 14 atomic tasks (T-19A01..A11 + T01..T02 + Z01) across three delivery phases α/β/γ; `tasks/phase-19.md` created; Phase 18 row updated In Progress (9/14 — T-18A01..A09 Done); PLAN v2.19.0 → v2.20.0; TASKS v2.15.0 → v2.16.0. Activation conditions met: Phase 18 T-18A01..A09 Done + l2-quantization-impl.md Stable v0.1.0 registered in INDEX v2.18.0.)
